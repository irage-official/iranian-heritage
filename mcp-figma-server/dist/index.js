...
mcpServer.tool("figma_get_file", {
    fileKey: z.string().min(1).describe("Figma file key"),
    depth: z.number().int().min(1).max(4).optional().describe("Optional depth 1-4"),
}, async ({ fileKey, depth }) => {
    const data = await figmaGet(`/files/${encodeURIComponent(fileKey)}`, depth ? { depth: String(depth) } : undefined);
    return { content: [{ type: "json", json: data }] };
});
mcpServer.tool("figma_get_nodes", {
    fileKey: z.string().min(1).describe("Figma file key"),
    nodeIds: z.array(z.string().min(1)).min(1).describe("Array of node IDs"),
}, async ({ fileKey, nodeIds }) => {
    const data = await figmaGet(`/files/${encodeURIComponent(fileKey)}/nodes`, { ids: nodeIds.join(",") });
    return { content: [{ type: "json", json: data }] };
});
mcpServer.tool("figma_get_images", {
    fileKey: z.string().min(1).describe("Figma file key"),
    nodeIds: z.array(z.string().min(1)).min(1).describe("Array of node IDs"),
